...
class MiniMaxH3Qwen3VLEncoder(nn.Module):
    ...
    ``_layerwise_offload_blocks_attrs`` points at the retained decoder layers so
    that ``--layerwise-offload-components text_encoder`` can stream them block by
    block. The path is dotted because the layers live under ``text_model``; the
    vision tower and embeddings stay device-resident.
    """

    _layerwise_offload_blocks_attrs: ClassVar[list[str]] = ["text_model.layers"]
