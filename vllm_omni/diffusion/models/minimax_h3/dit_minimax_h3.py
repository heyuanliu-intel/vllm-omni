...
#
# PORTED, NOT REWRITTEN. Model body is byte-identical to SGLang
# multimodal_gen @ 5d8a20b17 (runtime/models/dits/minimax_h3.py), including
# the Intel XPU TP=8 work (MergedColumnParallelLinear for packed qkv/fc1).
# Only imports were rewritten; see port_dit.py for the exact substitutions.
# Numerical parity against the SGLang implementation is REQUIRED before this
# is trusted: mis-sharded packed qkv degrades quality silently.
# SPDX-License-Identifier: Apache-2.0
...
from vllm_omni.diffusion.models.minimax_h3.compat import (
...
from vllm_omni.diffusion.models.minimax_h3.dit_config import (
    ...
    def forward(
        ...
        *,
        ...
        if sp_active:
            ...
            from vllm_omni.diffusion.models.minimax_h3.usp_stub import (
...
class MiniMaxH3DiTModel(CachableDiT):
    # Cache-DiT has no built-in adapter for this architecture, so the model has
    # to declare its own or enable_cache() raises. Pattern_3 is the single-stream
    # shape: blocks take hidden alone and return hidden alone (the other block
    # arguments are keyword-only and pass straight through).
    # NOTE: the CachableDiT base is a compat stand-in (compat.py) and carries no
    # caching behaviour -- this attribute is what actually wires Cache-DiT up.
    # check_forward_pattern is off because Cache-DiT matches patterns by argument
    # NAME: it looks for "hidden_states", while this block calls it "x". The shape
    # is genuinely Pattern_3 (hidden in, hidden out) -- only the name differs, and
    # renaming the block argument would mean editing ported model code.
    _cache_dit_adapter_config = CacheDiTAdapterConfig(
        block_forward_patterns={"blocks": ForwardPattern.Pattern_3},
        check_forward_pattern=False,
    )
    # regionally_compile() looks for this attribute and returns the model
    # UNCOMPILED (warning only) when it is missing -- so without it the port
    # runs eager no matter what enforce_eager / diffusion_compile_granularity
    # say. The name is matched against submodule CLASS names, so it has to be
    # the element type of self.blocks, not the attribute name.
    ...
    def __init__(
        ...
        *,
        config: MiniMaxH3DiTConfig | None = None,
        hf_config: dict[str, Any] | None = None,
    ) -> None:
        """Build the DiT from either the engine's config or an explicit one.

        The pipeline (and the engine behind it) constructs this as
        ``MiniMaxH3DiTModel(od_config, quant_config=...)`` and expects the
        architecture to come from the checkpoint's own
        ``transformer/config.json``. The older explicit
        ``config=``/``hf_config=`` form is still accepted so the contract tests
        and any hand-driven script keep working.

        Deriving the arch from the checkpoint rather than from hard-coded
        defaults is the point: the two agree exactly for the FL2VA checkpoint
        today, but a checkpoint that disagreed would otherwise be loaded under
        the wrong shape without any error.
        """
        if od_config is not None:
            if config is not None:
                raise TypeError("pass either od_config or config=, not both")
            tf_config = getattr(od_config, "tf_model_config", None)
            if tf_config is None:
                raise ValueError("od_config.tf_model_config is required")
            mapping = (
                tf_config.to_dict()
                if hasattr(tf_config, "to_dict")
                else dict(tf_config)
            )
            config = MiniMaxH3DiTConfig(
                arch_config=MiniMaxH3DiTArchConfig.from_mapping(mapping)
            )
            if hf_config is None:
                hf_config = dict(mapping)
            if quant_config is None:
                quant_config = getattr(od_config, "quantization_config", None)
        if config is None:
            raise TypeError("MiniMaxH3DiTModel needs od_config or config=")
    ...
    def load_weights(
        ...
        """Load a stream of (name, tensor) pairs; return the names consumed.

        This is the loader interface the engine's model loader and the
        pipeline-level ``load_weights`` drive. Sharding must go through each
        parameter's ``weight_loader``, never a plain ``copy_``: the checkpoint
        stores qkv and fc1 as single packed matrices, and the parallel linear's
        loader is what splits them into their logical sub-matrices ([q,k,v] and
        [gate,up]) *before* slicing by rank. A plain copy only happens to work
        at TP=1 and silently mis-shapes everything above it.
        """
        params: dict[str, torch.Tensor] = dict(self.named_parameters())
