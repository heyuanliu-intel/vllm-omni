...
def _accelerator_rng_devices(device: torch.device) -> list[torch.device]:
    """Devices whose RNG must be forked/seeded for a deterministic encode.

    Upstream hard-codes ``"cuda"`` here. On any other accelerator that check
    yields an empty list, so the seeding below silently becomes a no-op and the
    encode loses its determinism guard without any error. Accept whichever
    accelerator backend actually exposes ``manual_seed``.
    """
    backend = getattr(torch, device.type, None)
    if device.type != "cpu" and backend is not None and hasattr(backend, "manual_seed"):
        return [device]
    return []


def _seed_accelerator(device: torch.device, seed: int) -> None:
    backend = getattr(torch, device.type, None)
    if backend is None or not hasattr(backend, "manual_seed"):
        return
...
class MiniMaxH3VideoVAE(nn.Module, DistributedVaeMixin):
    ...
    # Entry points are dispatched through forward() so that
    # nn.Module.__call__ -- and therefore any offload hook registered on this
    # module -- runs first. See _CALLABLE_MODES for the allowed names.
    _CALLABLE_MODES = frozenset({"encode_image", "encode_video", "decode_latent"})

    def forward(self, mode: str, *args, **kwargs):
        if mode not in self._CALLABLE_MODES:
            raise ValueError(f"unknown video VAE mode {mode!r}; expected one of {sorted(self._CALLABLE_MODES)}")
    ...
    def encode_image(self, image: Image.Image) -> torch.Tensor:
        ...
        devices = _accelerator_rng_devices(parameter.device)
        try:
            with torch.random.fork_rng(
                ...
                for device in devices:
                    _seed_accelerator(device, MINIMAX_H3_KEYFRAME_ENCODE_SEED)
...
class MiniMaxH3AudioVAE(nn.Module):
    ...
    # Same dispatch contract as the video VAE; see MiniMaxH3VideoVAE.forward.
    _CALLABLE_MODES = frozenset({"encode_waveform", "decode_latent"})

    def forward(self, mode: str, *args, **kwargs):
        if mode not in self._CALLABLE_MODES:
            raise ValueError(f"unknown audio VAE mode {mode!r}; expected one of {sorted(self._CALLABLE_MODES)}")
        return getattr(self, mode)(*args, **kwargs)
