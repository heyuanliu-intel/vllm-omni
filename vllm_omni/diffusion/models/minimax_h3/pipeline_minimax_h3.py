...
from .dit_minimax_h3 import MiniMaxH3DiTModel
...
def _ar_mismatch_ratio(size: tuple[int, int], target_w: int, target_h: int) -> float:
    w, h = size
    target_ar = target_w / target_h
    return abs(w / h - target_ar) / target_ar


def _pad_to_aspect(image: Image.Image, target_w: int, target_h: int) -> Image.Image:
    """Letterbox: center the source on an RGB(128,128,128) canvas at the target
    aspect ratio without scaling the source pixels (doc §36.2 E4: padding keeps
    the full composition and a wide framing; cropping loses content and zooms).
    """
    w, h = image.size
    target_ar = target_w / target_h
    if w / h < target_ar:
        canvas_w, canvas_h = max(w, int(round(h * target_ar))), h
    else:
        canvas_w, canvas_h = w, max(h, int(round(w / target_ar)))
...
class MiniMaxH3Pipeline(
    ...
    _PROFILER_TARGETS: ClassVar[list[str]] = [
        ...
        # The three entries above only fire on the VIDEO-reference branch of
        # ref2va (forward() guards them with `raw_videos is not None`). The
        # IMAGE-reference branch runs _encode_visual_condition /
    ...
    def diffuse(
        ...
        *,
        ...
        logger.info(
            "[H3Pad] task=%s pad_seq_len=%s used=%d seq_len=%d",
            task,
            pad_seq_len,
    ...
    def forward(self, request: DiffusionRequestBatch) -> DiffusionOutput:
        ...
        if task == "fl2va" and image is not None:
            ...
            if os.getenv("H3_REF_LETTERBOX", "1") != "0":
                ...
                if mismatch > ar_tolerance:
                    ...
                    image = _pad_to_aspect(image, width, height)
                    logger.info(
                        "[H3Letterbox] task=ref2va in=%dx%d out=%dx%d target=%dx%d mismatch=%.4f",
                        in_w, in_h, image.width, image.height, width, height, mismatch,
                    )
                else:
                    logger.info(
                        "[H3Letterbox] task=ref2va in=%dx%d target=%dx%d mismatch=%.4f idempotent",
                        image.width, image.height, width, height, mismatch,
                    )
            else:
                logger.info("[H3Letterbox] task=ref2va disabled by H3_REF_LETTERBOX=0")
        ...
        pad_seq_len = extra.get("pad_seq_len")
